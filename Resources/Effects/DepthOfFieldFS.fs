...
uniform sampler2D u_Texture2;	// blur
uniform sampler2D u_Texture3;	// depth
...
{
	float depth = texture2D(u_Texture3, uv).x;
...
void main(void)
{
	...
	vec4 color1 = texture2D(u_Texture2, v_uv);
	...
	vec4 colorOriginal = texture2D(u_Texture1, v_uv);
	vec4 colorBlur = (2.0 * color1 + d2 * color2 + d3 * color3 + d4 * color4 + d5 * color5 +
				d6 * color6 + d7 * color7 + d8 * color8 + d9 * color9) / total;
